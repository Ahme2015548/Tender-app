...
const ForeignSuppliersList = ({ onEdit, onAdd, refreshTrigger }) => {
  ...
  const loadSuppliers = async () => {
    try {
      setLoading(true);
      const suppliersData = await ForeignSupplierService.getAllSuppliers();
  ...
  const handleDeleteConfirm = async (supplier) => {
    try {
      ...
      await SimpleTrashService.moveToTrash(supplier, 'foreignSuppliers');
      
      // Delete from original collection
      await ForeignSupplierService.deleteSupplier(supplier.id);
      
      // Log activity for foreign supplier deletion
      const currentUser = getCurrentUser();
      logActivity('task', `${currentUser.name} حذف مورد أجنبي`, `تم حذف المورد: ${supplier.name}`);
      
      await loadSuppliers();
      showSuccess(`تم نقل المورد الأجنبي للمهملات بنجاح: ${supplier.name}`, 'تم النقل للمهملات');
    } catch (err) {
      setError(err.message);
  ...
  return (
    <div className="card shadow-sm" data-list="foreign-suppliers">
...
export default ForeignSuppliersList;
