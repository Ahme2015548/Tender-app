...
const LocalProductsList = ({ refreshTrigger }) => {
  ...
  const {
    currentPage,
    totalPages,
    paginatedItems: paginatedLocalProducts,
    handlePageChange,
    resetPage,
    totalItems,
    itemsPerPage
  } = usePagination(filteredLocalProducts, 30);
  ...
  const handleDeleteClick = (localProduct) => {
    showConfirm(
      `هل أنت متأكد من حذف هذا المنتج المحلي؟\n\n${localProduct.name}`,
      () => handleDeleteConfirm(localProduct),
      'تأكيد حذف المنتج المحلي'
    );
  };

  const handleDeleteConfirm = async (localProduct) => {
    try {
      setDeleting(true);
      
      // Move to trash instead of permanent deletion
      await SimpleTrashService.moveToTrash(localProduct, 'localproducts');
      
      // Delete from original collection
      await LocalProductService.deleteLocalProduct(localProduct.id);
      
      // Log activity for local product deletion
      const currentUser = getCurrentUser();
      logActivity('task', `${currentUser.name} حذف منتج محلي`, `تم حذف المنتج المحلي: ${localProduct.name}`);
      
      await loadLocalProducts();
      showSuccess(`تم نقل المنتج المحلي للمهملات: ${localProduct.name}`, 'تم النقل للمهملات');
    } catch (err) {
      setError(err.message);
      showError(`فشل في نقل المنتج المحلي للمهملات: ${err.message}`, 'خطأ في النقل');
    } finally {
      setDeleting(false);
    }
  };
  ...
  return (
    <div className="card shadow-sm" data-list="local-products">
          ...
          <div className="col-lg-8">
            <div className="d-flex justify-content-end align-items-center gap-3">
              ...
              <button 
                ...
                onClick={() => navigate('/local-products/add')}
                style={{
                  borderRadius: '8px',
                  fontSize: '14px',
                  height: '44px',
      ...
      <div className="card-body p-0">
        {filteredLocalProducts.length === 0 ? (
          <div className="text-center py-5">
            ...
            {!searchTerm && (
              <button className="btn btn-primary" onClick={() => navigate('/local-products/add')}>
          ...
          <div className="table-responsive">
            <table className="table table-hover custom-striped mb-0">
              ...
              <tbody>
                {paginatedLocalProducts.map((product, index) => {
                      ...
                      <td className="text-center">
                        <button
                          ...
                          onClick={(e) => {
                            e.preventDefault();
                            e.stopPropagation();
                            console.log('=== LOCAL PRODUCT BUTTON CLICKED ===');
                            console.log('Product Name:', product.name);
                            console.log('Product ID:', product.id);
                            
                            if (product.id) {
                              const targetUrl = `/local-products/edit/${product.id}`;
                      ...
                      <td className="text-center">
                        <div className="btn-group btn-group-sm">
                          <button
                            ...
                            onClick={() => navigate(`/local-products/edit/${product.id}`)}
                            title="تعديل"
                          >
