...
export class UnifiedTenderItemsService {
  ...
  /**
   * Migrate localStorage tender items to Firebase
   * @param {string} tenderId - Tender ID
   * @param {Array} localStorageItems - Items from localStorage
   * @returns {Array} Migrated Firebase items
   */
  static async migrateLocalStorageToFirebase(tenderId, localStorageItems) {
    try {
      console.log('🔄 [UNIFIED] Migrating localStorage items to Firebase:', localStorageItems.length);
      
      if (!Array.isArray(localStorageItems) || localStorageItems.length === 0) {
        return [];
      }
      
      // Check what already exists in Firebase to prevent duplicates
      const existingFirebaseItems = await this.getTenderItems(tenderId);
      const existingKeys = new Set(
        existingFirebaseItems.map(item => `${item.materialInternalId}_${item.materialType}`)
      );
      
      const itemsToMigrate = localStorageItems.filter(item => {
        const itemKey = `${item.materialInternalId}_${item.materialType}`;
        return !existingKeys.has(itemKey);
      });
      
      console.log('📊 Migration analysis:', {
        totalLocalItems: localStorageItems.length,
        existingInFirebase: existingFirebaseItems.length,
        newItemsToMigrate: itemsToMigrate.length
      });
      
      if (itemsToMigrate.length === 0) {
        console.log('ℹ️ [UNIFIED] No new items to migrate');
        return existingFirebaseItems;
      }
      ...
      for (const item of itemsToMigrate) {
        try {
          // Get fresh material details for accurate migration
          let materialDetails = null;
          ...
          console.error('❌ [UNIFIED] Error migrating item:', item.materialName, itemError);
        }
      }
      
      if (migratedItems.length > 0) {
        await batch.commit();
        console.log('✅ [UNIFIED] Migrated items to Firebase:', migratedItems.length);
      }
      
      triggerDataSync();
      
      // Return combined existing + migrated items
      const allItems = await this.getTenderItems(tenderId);
      return allItems;
      
    } catch (error) {
      console.error('❌ [UNIFIED] Error migrating to Firebase:', error);
      throw new Error(`فشل في ترحيل البيانات إلى Firebase: ${error.message}`);
    }
  }
